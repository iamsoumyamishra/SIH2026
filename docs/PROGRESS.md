...
- **2026-08-30 (honest approvals)** — The approval note no longer fabricates decisions. Previously *any* document (e.g. an uploaded SIH deck) produced `MC-UNKNOWN`, `Items inspected: 0`, a fake `Maintenance SOP` reference, and a blanket "APPROVED for continued service". Now `make_approval_note` is data-driven: no extracted checklist → `REVIEW REQUIRED` verdict + "Manual review required" action + `Machine ID: Not identified in the document` (never `MC-UNKNOWN` nor fake approvals); no RAG hit → `No maintenance SOP retrieved from the knowledge base.`; failed items → `CONDITIONALLY APPROVED` with corrective scope. Verified against the empty-result DOCX; regression test added (`test_make_approval_note_inconclusive_never_approves`), 43/43 pytest green.
